...
from thiszigzag import PEAK, VALLEY


class TestIdentifyInitialPivot(TestCase):
    def test_strictly_increasing(self):
        data = np.linspace(1, 2., 10)
        self.assertEqual(thiszigzag.identify_initial_pivot(data, 0.1, -0.1),
                         VALLEY)
...
class TestPeakValleyPivots(TestCase):
    def test_guard_against_common_threshold_value_mistake(self):
        ...
        self.assertRaises(ValueError, thiszigzag.peak_valley_pivots,
                          data, 0.1, 0.1)

    def test_strictly_increasing(self):
        data = np.linspace(1, 10, 10)
        result = thiszigzag.peak_valley_pivots(data, 0.1, -0.1)
...
class TestSegmentReturn(TestCase):
    def test_strictly_increasing(self):
        ...
        pivots = thiszigzag.peak_valley_pivots(data, 0.1, -0.1)
        assert_array_almost_equal(thiszigzag.compute_segment_returns(data, pivots),
                                  np.array([99.0]))
...
class TestMaxDrawdown(TestCase):
    def test_strictly_increasing(self):
        data = np.linspace(1.0, 100.0, 10)
        self.assertEqual(thiszigzag.max_drawdown(data), 0.0)

    def test_strictly_decreasing(self):
        data = np.linspace(100.0, 1.0, 10)
        self.assertEqual(thiszigzag.max_drawdown(data), 0.99)

    def test_rise_fall_rise_drawdown(self):
        data = np.array([1.0, 1.05, 1.1, 1.0, 0.9, 1.5])
        self.assertAlmostEqual(thiszigzag.max_drawdown(data), 0.18181818181818188)


class TestPivotsToModes(TestCase):
    def test_pivots_to_modes(self):
        data = np.array([1, 0, 0, 0, -1, 0, 0, 1, -1, 0, 1])
        result = thiszigzag.pivots_to_modes(data)
...
def test_peak_valley_pivots_pandas_compat():
    ...
    got = thiszigzag.peak_valley_pivots(df.X, 0.2, -0.2)
